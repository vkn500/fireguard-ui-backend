...
def send_email_thread(status, severity, timestamp):
    ...
    print("📧 Email Alert Triggered (Threaded)")
...
while True:
    ...
    if severity >= 2:
        ...
        # send email only if never sent or cooldown passed
        ...
        stop_alarm()
        email_sent = False   # Reset when fire ends
    # -----------------------------------------
    ...
    cv2.imshow("🔥 Webcam Fire Detection | Alarm + Email (Anti-Spam)", frame)
